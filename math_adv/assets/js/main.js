...
function checkAuthAndRedirect(role) {
    ...
    const loginUrl = 'portal/login.php';
    
    // Create a modal for portal access
    const modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header" style="background: var(--gradient-primary); color: white;">
                        <h5 class="modal-title">
                            <i class="fas fa-shield-alt me-2"></i>Accés al Portal ${role.charAt(0).toUpperCase() + role.slice(1)}
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body text-center p-4">
                        <div class="portal-access-info">
                            <div class="portal-icon mb-3" style="font-size: 3rem; color: var(--primary-color);">
                                ${getPortalIcon(role)}
                            </div>
                            <h6 class="fw-bold mb-3">Portal ${getPortalTitle(role)}</h6>
                            <p class="text-muted mb-4">
                                Per accedir al portal necessites les teves credencials d'usuari. 
                                Si encara no tens compte, contacta amb l'administració.
                            </p>
                            <div class="d-grid gap-2">
                                <a href="${loginUrl}" class="btn btn-primary btn-lg">
                                    <i class="fas fa-sign-in-alt me-2"></i>Iniciar Sessió
                                </a>
                                <button type="button" class="btn btn-outline-secondary" onclick="showDemoInfo('${role}')">
                                    <i class="fas fa-eye me-2"></i>Veure Demo
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('portalModal'));
    modal.show();
}
